...
const ENV_PREFIX = 'QORTIUM_RECIPES';
const DEFAULT_NODE_API_URL = 'http://127.0.0.1:24891';
const DEFAULT_NAME = 'Recipes';
const DEFAULT_IDENTIFIER = 'Recipes';
const DEFAULT_TITLE = 'Recipes';
const DEFAULT_DESCRIPTION = 'Publish, browse, and scale community recipes on QDN.';
...
async function ensureNameRegistered(name, account) {
  ...
  const rawRegisterBytes58 = buildRegisterNameRawBytes58({
    ...
    data: JSON.stringify({
      app: DEFAULT_TITLE,
      purpose: 'QDN app preview for Recipes',
    }),
  });
